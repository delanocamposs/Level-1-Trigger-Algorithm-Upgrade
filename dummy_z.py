...
R_MB_CM={1:402.0,2:490.5,3:597.5,4:700.0}
ZRES_CONV=65536.0/1500.0

#global dictionaries across all events
gen_eta_glob, gen_z_glob, gen_pt_glob={st: [] for st in range(1, 4)}, {st: [] for st in range(1, 4)}, {st: [] for st in range(1, 4)}
stub_eta_glob, stub_z_glob={st: [] for st in range(1, 4)}, {st: [] for st in range(1, 4)}
delta_z_glob={st:[] for st in range(1,4)}
...
for i, event in enumerate(events):
    ...
    gen_z_event_by_sta={st: [] for st in range(1, 4)}
    gen_pt_event_by_sta={st: [] for st in range(1, 4)} #need this to keep stations and pT organized after matching

    #storing per event information about stub hits organized by station. 
    stub_eta_event={st: [] for st in range(1, 4)} #filled with stub eta values from: station and z value.
    stub_indices_event={} #filled with indices of matched stubs to gen muons.
    stub_z_event={st: [] for st in range(1, 4)} #filled with stub z value from knowing base z value from theta primitive.  
    stub_eta_matched={st:len(gen_eta_event)*[None] for st in range(1,4)} #new stub eta values after matching. IMPORTANT TO KEEP NONE TYPES.
    stub_z_matched={st:len(gen_eta_event)*[None] for st in range(1,4)} 
    stub_z_matched_by_sta={st:[] for st in range(1,4)} 
    
    delta_z_event={st: [] for st in range(1, 4)}

    #loop through stub hits in each event to obtain:
    #stub eta, stub theta and store in above arrays
    for stub in container:
        st=int(stub.stNum())
        stub_z_event[st].append(float(stub.z()/ZRES_CONV))
        stub_eta_event[st].append(eta_from_z(float(stub.z()/ZRES_CONV),R_MB_CM[st]))
    ...
    for st in (1, 2, 3):
        ...
        for mu_idx, stub_idx in enumerate(match_idx): #mu_idx is index of gen muon. stub_idx is best stub index.
            ...
            gen_z_val=mu_vz+R_MB_CM[st]*np.sinh(mu_eta)
            #print("station: ", st)
            #print("mu idx: ", mu_idx)
            #print("stub idx: ", stub_idx)
    ...
    for st in (1,2,3):  
        #gen_eta_glob[st].extend()
        gen_z_glob[st].extend(gen_z_event_by_sta[st])
        gen_pt_glob[st].extend(gen_pt_event_by_sta[st])

        #stub_eta_glob[st].extend()
        stub_z_glob[st].extend(stub_z_matched_by_sta[st])

    #if len(gen_eta_event)==2:
        #print("gen_pt_event: ", gen_pt_event)
        #print("gen_eta_event: ", gen_eta_event)
        #print("stub_eta_event BEFORE: ", stub_eta_event)
        #print("stub_indices_event: ", stub_indices_event)
        #print("stub_eta_event AFTER: ", stub_eta_matched)
        #print("stub_z_event BEFORE: ", stub_z_event)
        #print("stub_indices_event: ", stub_indices_event)
        #print("stub_z_event AFTER: ", stub_z_matched)
        #print("stub_z_event AFTER: ", stub_z_matched_by_sta)
        #print("gen_pt AFTER: ", gen_pt_event_by_sta)
        #print("gen_z_event: ", gen_z_event_by_sta)
        #print("----------------------")
           
sigma_hists = {}

for st in range(1,4):
    delta_z_glob[st]=np.array(gen_z_glob[st])-np.array(stub_z_glob[st])
